...
import collection.{Map => ParentMap}
import collection.immutable.{ListMap => IListMap}
...
final class LNode[K, V](final val listmap: IListMap[K, V])
extends MainNode[K, V] {
  def this(k: K, v: V) = this(IListMap(k -> v))
  def this(k1: K, v1: V, k2: K, v2: V) = this(IListMap(k1 -> v1, k2 -> v2))
...
extends ConcurrentTrieBase[K, V] with ConcurrentMap[K, V] {
  ...
  */
  ...
  @tailrec final def readOnlySnapshot(): ParentMap[K, V] = {
    val r = RDCSS_READ_ROOT()
...
object MainNode {
  ...
  final val updater: AtomicReferenceFieldUpdater[MainNode[_, _], MainNode[_, _]] = AtomicReferenceFieldUpdater.newUpdater(classOf[MainNode[_, _]], classOf[MainNode[_, _]], "prev")
...
abstract class CNodeBase[K, V] extends MainNode[K, V] {
    ...
    var bitmap = 0
    var array : Array[BasicNode] = null
}

abstract class ConcurrentTrieBase[K, V] {
 
    @volatile var root : AnyRef = null
}

final class Gen {
}

abstract class INodeBase[K, V](generation : Gen) extends BasicNode {
   
    @volatile var mainnode : MainNode[K, V] = null
    var gen : Gen = generation
...
object INodeBase {
	final val updater : AtomicReferenceFieldUpdater[INodeBase[_,_], MainNode[_,_]] = AtomicReferenceFieldUpdater.newUpdater(classOf[INodeBase[_,_]], classOf[MainNode[_,_]], "mainnode")
	final var RESTART : AnyRef = new Object()
}
